<template>
  <div class="animate-in fade-in duration-300">
      ...
      <div class="flex items-center gap-2">
        <div class="bg-white/5 p-1 rounded-xl flex gap-1">
          <button
            ...
            class="px-4 py-2 rounded-lg text-xs font-semibold uppercase tracking-wide transition-all"
            :class="viewMode === 'month' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-slate-400 hover:text-white'"
          >
            Month
          </button>
          <button
            @click="viewMode = 'week'"
            class="px-4 py-2 rounded-lg text-xs font-semibold uppercase tracking-wide transition-all"
            :class="viewMode === 'week' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-slate-400 hover:text-white'"
          >
            Week
          </button>
        </div>
        ...
        <button
          ...
          class="h-10 px-3 rounded-xl text-xs font-medium transition-all active:scale-95 flex items-center gap-2"
          :class="showHeatmap ? 'bg-primary/10 text-primary border border-primary/20' : 'bg-white/[0.03] text-slate-400 border border-white/[0.06] hover:bg-white/[0.07]'"
        >
        ...
        <button
          @click="goToToday"
          class="h-10 px-4 rounded-xl bg-white/[0.03] hover:bg-white/[0.07] text-slate-300 hover:text-white border border-white/[0.06] text-xs font-medium tracking-wide transition-all active:scale-95"
        >
          Today
        </button>
      </div>
    ...
    <div class="flex items-center gap-x-4 gap-y-2 mb-6 flex-wrap pb-4 border-b border-white/[0.04] overflow-x-auto">
      ...
      <div v-if="viewMode === 'month'" class="flex items-center gap-2 text-[11px] text-slate-500 font-medium whitespace-nowrap">
        <span class="w-3 h-3 rounded bg-emerald-500/20 border border-emerald-500/30"></span> Light
      </div>
      <div v-if="viewMode === 'month'" class="flex items-center gap-2 text-[11px] text-slate-500 font-medium whitespace-nowrap">
        <span class="w-3 h-3 rounded bg-amber-500/20 border border-amber-500/30"></span> Medium
      </div>
      <div v-if="viewMode === 'month'" class="flex items-center gap-2 text-[11px] text-slate-500 font-medium whitespace-nowrap">
        <span class="w-3 h-3 rounded bg-rose-500/20 border border-rose-500/30"></span> Heavy
      </div>
    </div>

    <!-- ===== Views + Sidebar (responsive flex layout) ===== -->
    <div class="flex flex-col xl:flex-row gap-6 items-start">
      <!-- Calendar area -->
      <div class="flex-1 min-w-0 w-full">
        <div v-if="loading" class="space-y-2">
          <div class="grid grid-cols-7 gap-1">
            <div v-for="i in (viewMode === 'month' ? 42 : 7)" :key="i" class="h-24 bg-white/[0.02] border border-white/[0.04] animate-pulse rounded-xl"></div>
          </div>
        </div>

        <div v-else-if="allItems.length === 0" class="text-center py-20">
          <UIcon name="i-heroicons-calendar" class="w-10 h-10 text-slate-600 mx-auto mb-3" />
          <p class="text-slate-400 font-medium">Nothing scheduled yet.</p>
          <p class="text-slate-500 text-xs mt-1">Add milestones, tasks, or invoices to see them here.</p>
        </div>
        ...
        <template v-else>
          ...
          <template v-if="viewMode === 'month'">
            <div class="grid grid-cols-7 mb-2">
              <div v-for="day in ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat']" :key="day" class="py-1 text-center text-[10px] font-bold uppercase tracking-widest text-slate-500">
            ...
            <div class="grid grid-cols-7 gap-1 bg-white/[0.01] p-1 rounded-2xl border border-white/[0.04]">
              ...
              >
                <div class="flex justify-between items-start mb-1">
                  ...
                  <div v-if="(itemsByDate[toKey(day.date)] || []).length > 3" class="w-1.5 h-1.5 rounded-full bg-slate-500 opacity-60 mt-1"></div>
                </div>
                <div class="space-y-0.5 flex-1 flex flex-col justify-end">
                  <template v-for="(item, i) in (itemsByDate[toKey(day.date)] || [])" :key="item.id">
                    <div v-if="i < 3" class="flex items-center gap-1 px-1 py-0.5 rounded-md text-[9px] sm:text-[10px] font-medium truncate border transition-all" :class="chipClass(item)">
                      <span class="w-1 h-1 rounded-full shrink-0" :class="dotClass(item)"></span>
                      <span class="truncate" :class="{ 'line-through opacity-40': statusDoneClass(item) }">{{ item.title }}</span>
                    </div>
                  </template>
                  <div v-if="(itemsByDate[toKey(day.date)] || []).length > 3" class="text-[8px] sm:text-[9px] text-slate-500 font-semibold px-1 mt-0.5">
          ...
          <template v-else>
            <div class="grid grid-cols-7 gap-2">
              <div
                ...
                :key="toKey(day)"
                @click="selectDay(day)"
                class="bg-white/[0.02] border rounded-xl p-3 cursor-pointer hover:bg-white/[0.05] transition-all"
                :class="isSelected(day) ? 'border-primary/40 ring-1 ring-primary/30' : 'border-white/[0.04]'"
              >
                <div class="text-center mb-3">
                  <p class="text-[10px] uppercase tracking-wider text-slate-500">{{ day.toLocaleDateString(undefined, { weekday: 'short' }) }}</p>
                  <p class="text-lg font-bold" :class="isToday(day) ? 'text-primary' : 'text-white'">{{ day.getDate() }}</p>
                </div>
                <div class="space-y-1.5">
                  <template v-for="item in (itemsByDate[toKey(day)] || [])" :key="item.id">
                    <div class="flex items-center gap-1 px-1.5 py-1 rounded-md text-[10px] font-medium truncate border transition-all" :class="chipClass(item)">
                      <span class="w-1 h-1 rounded-full shrink-0" :class="dotClass(item)"></span>
                      <span class="truncate" :class="{ 'line-through opacity-40': statusDoneClass(item) }">{{ item.title }}</span>
                    </div>
                  </template>
                  <p v-if="(itemsByDate[toKey(day)] || []).length === 0" class="text-slate-600 text-xs text-center">—</p>
                </div>
              </div>
            </div>
          </template>
      ...
      <Transition name="panel">
        <div
          v-if="selectedDay && !isSmallScreen"
          class="hidden xl:block w-80 shrink-0 bg-white/[0.03] border border-white/6 rounded-2xl p-5 self-start"
        >
          <div class="flex items-start justify-between mb-4 pb-3 border-b border-white/5">
            ...
            <button @click="selectedDay = null" class="w-8 h-8 rounded-xl flex items-center justify-center text-slate-500 hover:text-white hover:bg-white/8 transition-all">
              <UIcon name="i-heroicons-x-mark" class="w-5 h-5" />
            </button>
          </div>
          <div v-if="selectedDayItems.length === 0" class="py-12 text-center text-slate-500 text-xs flex flex-col items-center gap-2">
            <UIcon name="i-heroicons-calendar" class="w-6 h-6 text-slate-600" />
            <p>No activities on this day.</p>
          </div>
          <div v-else class="space-y-3 max-h-[70vh] overflow-y-auto pr-1 custom-scrollbar">
            <div v-for="item in selectedDayItems" :key="item.id" class="bg-white/[0.03] border border-white/6 rounded-xl p-4 hover:border-white/10 transition-all">
              <div class="flex items-center gap-2 mb-2">
                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[10px] font-semibold uppercase border" :class="panelChipClass(item)">
                  <UIcon :name="itemIcon(item)" class="w-3 h-3 shrink-0" />
                  {{ typeLabel(item) }}
                </span>
              </div>
              <p class="text-sm font-medium mb-2" :class="[statusDoneClass(item) ? 'line-through text-slate-500' : 'text-white']">{{ item.title }}</p>
              ...
              <div v-if="item.projectName || item.clientName" class="flex items-center gap-x-2 gap-y-1 mt-2 flex-wrap text-xs">
                <NuxtLink v-if="item.projectId" :to="`/projects/${item.projectId}`" class="text-slate-400 hover:text-primary transition-colors flex items-center gap-1" @click.stop>
                ...
                <NuxtLink v-if="item.clientId" :to="`/clients/${item.clientId}`" class="text-slate-400 hover:text-primary transition-colors flex items-center gap-1" @click.stop>
              ...
              <div v-if="statusDoneClass(item)" class="mt-2 flex items-center gap-1 text-[10px] text-emerald-400 font-medium">
    ...
    <Teleport to="body">
      <Transition name="sheet">
        ...
        >
          ...
          <div class="relative w-full bg-[#0d1525] border border-white/8 rounded-t-3xl max-h-[85vh] overflow-y-auto p-5 shadow-2xl">
...
const checkScreen = () => { isSmallScreen.value = window.innerWidth < 1280 }
...
const fetchData = async () => {
  loading.value = true
  try {
    let mQuery = supabase.from('milestones').select('id, title, due_date, status, project_id, projects(id, name, clients(id, name))').not('due_date', 'is', null)
    if (props.projectId) mQuery = mQuery.eq('project_id', props.projectId)
    const { data: mData } = await mQuery
    milestones.value = mData || []

    let tQuery = supabase.from('tasks').select('id, title, due_date, status, priority, project_id, milestone_id, milestones(id, title, projects(id, name, clients(id, name)))').not('due_date', 'is', null)
    if (props.projectId) tQuery = tQuery.eq('project_id', props.projectId)
    const { data: tData } = await tQuery
    tasks.value = tData || []

    let pQuery = supabase.from('projects').select('id, name, start_date, end_date, clients(id, name)').or('start_date.not.is.null,end_date.not.is.null')
    if (props.projectId) pQuery = pQuery.eq('id', props.projectId)
    const { data: pData } = await pQuery
    projects.value = pData || []

    let iQuery = supabase.from('retainers').select('id, title, invoice_number, amount, currency, status, due_date, clients(id, name), invoice_items(*)').not('due_date', 'is', null).in('status', ['pending', 'overdue'])
...
const allItems = computed(() => {
  const items = []
  milestones.value.forEach(m => items.push({
  ...
  projects.value.forEach(p => {
    ...
    if (p.end_date) items.push({
      id: `project-end-${p.id}`, date: p.end_date, type: 'project_end', title: p.name, status: 'active',
      projectId: p.id, projectName: p.name, clientId: p.clients?.id, clientName: p.clients?.name
    })
  })
  invoices.value.forEach(inv => {
    const total = inv.invoice_items?.length ? inv.invoice_items.reduce((s, i) => s + Number(i.quantity) * Number(i.unit_rate), 0) : Number(inv.amount)
    items.push({
...
const itemsByDate = computed(() => {
  const map = {}
  allItems.value.forEach(item => { if (!map[item.date]) map[item.date] = []; map[item.date].push(item) })
  return map
})
...
const isSelected = (d) => selectedDay.value === toKey(d)

const periodLabel = computed(() => {
  if (viewMode.value === 'month') return currentDate.value.toLocaleDateString(undefined, { month: 'long', year: 'numeric' })
  const monday = weekDays.value[0]
  const sunday = weekDays.value[6]
...
const selectedDayItems = computed(() => selectedDay.value ? (itemsByDate.value[selectedDay.value] ?? []) : [])
const selectedDayLabel = computed(() => {
  if (!selectedDay.value) return ''
  return new Date(selectedDay.value + 'T00:00:00').toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })
})
...
const dotClass = (item) => {
  ...
  if (item.type === 'invoice_due') return item.status === 'overdue' ? 'bg-rose-400' : 'bg-emerald-400'
  return 'bg-slate-500'
}
const chipClass = (item) => {
  ...
  if (item.type === 'invoice_due') return item.status === 'overdue' ? 'bg-rose-500/5 text-rose-400 border-rose-500/15' : 'bg-emerald-500/5 text-emerald-400 border-emerald-500/15'
  return 'bg-slate-500/5 text-slate-400 border-slate-500/10'
}
const panelChipClass = (item) => {
  ...
  if (item.type === 'invoice_due') return item.status === 'overdue' ? 'text-rose-400 bg-rose-400/10 border-rose-400/20' : 'text-emerald-400 bg-emerald-400/10 border-emerald-400/20'
  return 'text-slate-400 bg-slate-500/10 border-slate-500/20'
}
const itemIcon = (item) => {
...
const statusDoneClass = (item) => item.status === 'done' || item.status === 'complete' || item.status === 'paid'
...
.panel-enter-from, .panel-leave-to { opacity: 0; transform: scale(0.98); }
...
.sheet-enter-active > div:last-child { transition: transform 0.3s cubic-bezier(0.32, 0.72, 0, 1); }
.sheet-enter-to > div:last-child { transform: translateY(0); }
.sheet-leave-from > div:last-child { transform: translateY(0); }
.sheet-leave-active > div:last-child { transition: transform 0.2s ease-in; }
