...
// We only need the state. The Composable handles the auto-fetching now.
    ...
    <div class="flex-1 flex relative overflow-hidden">
      ...
      <aside
        :class="[
          ...
          'fixed inset-y-0 left-0 z-50 bg-secondary border-r border-white/5 flex flex-col transition-all duration-300 ease-in-out w-64 shadow-2xl lg:shadow-none'
        ]"
      >
        <div class="h-16 flex-none flex items-center justify-between px-4 border-b border-white/5 relative overflow-hidden">
          ...
          <div class="flex-1 flex items-center justify-center relative h-full">
             ...
             <NuxtLink to="/" @click="handleNavClick" class="transition-all duration-300 absolute inset-0 flex items-center justify-center"
               ...
               <img src="/img/clientbaselogo-min.png" alt="Client Base" class="h-12 w-auto object-contain drop-shadow-lg" />
             </NuxtLink>
          </div>

          <button @click="isMenuOpen = false" class="lg:hidden absolute right-4 text-gray-400 hover:text-white">
